...
resource "aws_vpc" "vpc" {
  ...
  tags = merge(var.external_tags, map("Name", "My_VPC"))
}
...
resource "aws_subnet" "public" {
  ...
  tags = merge(var.external_tags, map
...
resource "aws_internet_gateway" "igw" {
  ...
  tags = merge(var.external_tags, map("Name", "IGW_${aws_vpc.vpc.id}"))
}
...
resource "aws_route_table" "route_tables" {
  ...
  tags = merge(
  var.external_tags, map("Name", "${var.route_tables_names[count.index]}_RTB_${aws_vpc.vpc.id}"))
}
